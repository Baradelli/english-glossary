...
- _Acumular observações_. Em qualquer verbete — inclusive durante a revisão final de uma prova — o usuário pode anexar uma observação editável sem substituir o contexto anterior. Essas observações passam a informar prompts futuros. Aceite: duas inclusões preservam a ordem e sobrevivem a exportação/restauração.
...
**Fluxo B — Revisão por spaced repetition.** _(A tela dedicada de revisão foi aposentada — ADR-010; a revisão hoje acontece ao responder provas. O agendamento SM-2 abaixo continua íntegro.)_

- _Produzir revisões_. Responder uma questão de prova gera uma revisão SM-2 da palavra: acerto → qualidade 5, erro → 2. O sistema recalcula intervalo, fator de facilidade e `nextReview` e grava o `ReviewLog`. Aceite: dado um histórico de avaliações, os intervalos seguem o SM-2 de forma determinística (testável sem UI).
- _Agendar_. As palavras com `nextReview <= agora` são as "devidas" — dado exibido na faixa "hoje" do Painel e usado como peso na seleção da prova de vocabulário. Aceite: a due-ness respeita as datas de agendamento (compara instantes, não dia civil).
...
- **Word** — `id`, `term`, `definitionEn`, `definitionPt`, `examples` (frases **autorais** que você escreve ao cadastrar — seu domínio ativo; distintas da frase real da fonte, que vive em `WordSighting.contextSentence`), `observations` (JSON de contextos adicionados incrementalmente pelo estudante) e campos SRS: `easeFactor`, `intervalDays`, `repetitions`, `nextReview`, `createdAt`. O **estado** (nova/aprendendo/dominada) **não é coluna** — é derivado dos campos SRS (ver §6.1).
...
- **ReviewLog** — histórico de cada avaliação SRS: `wordId`, `quality` (0–5), `reviewedAt`, snapshot do intervalo resultante. Alimentado tanto pela **sessão de revisão** (4 botões Errei/Difícil/Bom/Fácil → quality 1/3/4/5) quanto pelo fechamento de **provas** (acerto → 5, erro → 2). Permite reconstruir/auditar o agendamento e alimenta o heatmap/streak do Painel.
- **Exam** — prova com dois mecanismos. **Quiz** (tipos `semanal`, `vocabulario`, `pratica`, ADR-009): a IA escreve as questões de múltipla escolha, respondidas questão a questão dentro do app; `status` `em_andamento` → `finalizada`, `score` computado **localmente** pelo app, `finishedAt`, `practiceOfId?` (prova de prática aponta a de origem). **Fluxo de dois turnos** (tipo `compreensao` e registros legados): `status` `gerada` → `respondida` → `corrigida` com os artefatos de texto `promptText`, `answersText`, `correctionPrompt`, `resultJson` validado por Zod. Ambos ligam-se às palavras avaliadas via **ExamWord** (`examId`, `wordId`, `correct`) e atualizam o SRS + `ReviewLog` numa transação.
- **ExamQuestion** — questão de uma prova: `examId`, `wordId`, `position`, `type` (`ai_context` nos quizzes novos; `mc_en_pt` | `mc_pt_en` | `typed` | `cloze` sobrevivem em provas geradas pelo motor local aposentado — ADR-007), `prompt`, `options?` (JSON), `optionExplanations?` (JSON alinhado por índice com as alternativas), `correctIndex?`, `correctAnswer?`, `contextSentence?`, `explanation?` (razão da correta usada no feedback imediato e como fallback legado), `userAnswer?`, `isCorrect?`, `answeredAt?`. Invariante: **exatamente 1 questão por verbete por prova** (`@@unique([examId, wordId])`) — preserva a unicidade de `ExamWord`. Respostas são persistidas questão a questão, então a prova é retomável.

Os pontos que exigem cuidado: WordSighting (N:N com atributo), as transações de fechamento de prova (`finishQuiz`/`submitCorrection`, que atualizam múltiplas Words + logs de forma all-or-nothing), e as duas máquinas de estado de `Exam` (o quiz só grava `score`/`ExamWord` na transição `em_andamento` → `finalizada`; o fluxo legado, na transição para `corrigida` após validação Zod).
...
> **Nota (ADR-007):** desde a introdução do motor local de questões, os templates 1 e 2 abaixo foram **aposentados** — revisão semanal e prova de vocabulário são geradas e corrigidas pelo próprio app, sem IA. O fluxo de dois turnos descrito aqui permanece vivo **apenas para a prova de compreensão** (template 3) e para renderizar provas antigas. Há ainda um prompt novo de **enriquecimento opcional** (`buildQuizContextQuestionsPrompt`): com chave de API configurada, a IA pode substituir até 5 questões de múltipla escolha por questões de uso-em-contexto (JSON validado por Zod; qualquer falha → fallback silencioso para as questões locais).
...
| Testes              | Vitest                                                                    | TDD nas regras de domínio (SM-2, prompts, parse, motor de questões, insights)                                                                                                         |
| Gráficos            | **Recharts** (linha/área/barras) + heatmap próprio                        | Lib mais popular de charts para React (ADR-008); heatmap de calendário não existe em Recharts → grade de divs própria                                                                 |
...
### ADR-007: Motor local de questões com enriquecimento opcional por IA (vs. prova via IA externa) — _superado pelo ADR-009_

> **Status: superado pelo ADR-009.** O motor local de questões foi aposentado; a IA passou a gerar a prova inteira. As provas antigas geradas por este motor continuam legíveis e corrigíveis (os tipos `typed`/`cloze` seguem no modelo e na correção). O restante deste ADR fica como registro histórico da decisão original.

- **Contexto:** as provas de revisão semanal e de vocabulário nasceram como fluxo de dois turnos copiar/colar com IA externa (ADR-001). Na prática, a prova acontecia fora do app: nenhuma questão renderizada, nenhum gabarito verificável, nota atribuída pela IA, quatro passos manuais de cópia — o valor central da feature ficava terceirizado e sem vida.
- **Opções:** **A) Motor local** — o app gera questões estruturadas do próprio glossário (múltipla escolha com distratores do glossário, resposta digitada com tolerância a typo, cloze sobre a frase real da fonte), o usuário responde questão a questão dentro do app e a correção/nota é computada pelo app; IA opcional apenas enriquece. **B) Quiz in-app gerado por IA via API** — questões melhores, mas a feature passa a exigir chave + rede, quebrando o offline-first. **C) Manter copiar/colar** e só polir a UX.
- **Fator decisivo:** offline-first é restrição de produto (PRODUCT.md) e correção verificável é pré-requisito para alimentar o SM-2 com honestidade. A geração determinística (semente injetável) ainda torna o motor totalmente testável — alinhado à barra de portfólio.
- **Decisão:** **A.** Novo módulo puro `src/domain/quiz/` (gerador com rotação de tipos e degradação, distratores com filtro anti-sinônimo, Damerau-Levenshtein ≤ 1 para digitadas, flexões conservadoras no cloze), modelo `ExamQuestion` persistido questão a questão (prova retomável), `score = round(100·acertos/total)` computado pelo app, e prova de **prática** (`pratica`) só com os erros. Com chave configurada, `enrichWithAi` substitui até 5 questões por uso-em-contexto gerado por IA, com fallback silencioso. O fluxo de dois turnos permanece **apenas** para compreensão de fonte (que exige entender o conteúdo) e para provas antigas.
- **Consequências:** prova funciona 100% offline e a IA vira bônus, não dependência; `buildWeeklyReviewPrompt`/`buildVocabularyExamPrompt` foram aposentados; o schema ganhou `ExamQuestion`/`finishedAt`/`practiceOfId` e dois status novos (`em_andamento`/`finalizada`) — reverter exigiria nova migração e reescrita da UI de provas, então a decisão é efetivamente estrutural.

### ADR-008: Recharts para os gráficos do Painel (vs. SVG próprio ou outras libs)

- **Contexto:** o Painel ganhou visualizações temporais (crescimento do vocabulário, previsão de revisões, tendência de notas) além do heatmap de atividade. O plano original propunha SVG server-rendered próprio, zero dependência; o usuário preferiu explicitamente uma biblioteca gratuita e consagrada.
- **Opções:** **A) Recharts** — MIT, a lib de charts mais popular do ecossistema React, declarativa, tooltips/eixos prontos. **B) SVG próprio** — zero dependência, porém reimplementa eixo/tooltip/responsividade à mão. **C) chart.js/visx/nivo** — alternativas viáveis, menos idiomáticas (chart.js é canvas/imperativo; visx é low-level; nivo é mais pesada).
- **Fator decisivo:** preferência do usuário + velocidade de manutenção; a dependência entra via npm e é empacotada no instalador, preservando o offline-first (nada de CDN).
- **Decisão:** **A.** Recharts para linha/área/barras, como client components com dados serializáveis; tema claro/escuro via CSS variables `--chart-*` definidas em `globals.css` (`:root` e `html.dark`) e tooltips customizados (o default da lib tem fundo fixo que quebraria o dark). O **heatmap continua grade de divs própria** — Recharts não tem calendar heatmap e a grade é o padrão da indústria para esse formato.
- **Consequências:** ~120 kB a mais no bundle da home (irrelevante em app local); gráficos novos ficam baratos de adicionar; se a lib um dia estagnar, os dados já saem prontos das funções puras de `src/domain/insights/` — trocar a camada de apresentação não toca domínio nem application.

### ADR-009: A IA gera a prova inteira, só múltipla escolha (vs. motor local do ADR-007)

- **Contexto:** o motor local (ADR-007) montava as questões com regras determinísticas — rotação de tipos, distratores por proximidade de comprimento, flexões de cloze. Na prática as questões saíam **engessadas e artificiais** ("Escreva em inglês o verbete que significa: …", cloze com flexão mecânica que às vezes mascarava a palavra errada), longe do objetivo de _ajudar a aprender_. Distratores tirados do glossário por heurística eram frequentemente óbvios ou, pior, ambíguos.
- **Opções:** **A) IA gera a prova toda, só múltipla escolha** — questões escritas por um professor-modelo, 4 alternativas, uma correta, com explicação; o app escolhe os verbetes, valida o JSON, embaralha as alternativas e corrige localmente. **B) Manter o motor local** e só melhorar as heurísticas. **C) Híbrido** (motor local + enriquecimento por IA, o desenho anterior) — mas isso mantinha as questões engessadas como caso base.
- **Fator decisivo:** a qualidade pedagógica das questões é o valor central da feature, e é justamente onde a heurística local não chega. Restringir a **só múltipla escolha** elimina a superfície de erro de correção (comparar índice é trivial e à prova de ambiguidade), então terceirizar a _geração_ para a IA não custa a _correção verificável_ — o app continua dono da nota e do SRS. O preço é exigir chave de API para gerar provas; aceitável porque a captura, a revisão (SRS) e o painel seguem 100% offline — só as provas passam a depender da IA.
- **Decisão:** **A.** `buildQuizGenerationPrompt` pede uma questão de múltipla escolha por verbete (enunciado, 4 alternativas, índice correto e 4 `optionExplanations` em PT); `parseAiQuiz` valida contra o schema Zod; `buildAiQuizQuestions` (puro, semente injetável) casa cada item ao verbete, descarta itens desonestos (alternativas vazias/duplicadas, termo sem correspondência), **reembaralha pares alternativa + explicação** (o índice correto da IA nunca vaza pela posição) e numera as questões. A app tenta a geração **duas vezes** antes de falhar com mensagem amigável; timeout de 90 s. Correção, nota e prova de prática permanecem locais e inalterados; a prática agora gera questões **novas** para os erros. O motor local (`generator.ts`, `cloze.ts`) foi removido.
- **Consequências:** provas passam a exigir chave de API (a UI de provas avisa e desabilita os botões sem chave); a revisão final é expansível e explica por que cada opção está certa ou errada; o schema ganhou `ExamQuestion.optionExplanations` por migração aditiva. Provas antigas continuam legíveis com fallback honesto para a explicação geral. Uma explicação pode ser editada e salva explicitamente como observação do verbete, passando a enriquecer provas futuras.

### ADR-010: Aposentar a tela dedicada de revisão (a revisão acontece nas provas)

- **Contexto:** com o quiz in-app (ADR-007, depois ADR-009), toda prova finalizada já roda o SM-2 e grava `ReviewLog` por palavra (acerto → 5, erro → 2). A tela `/review` — sessão card-a-card com virada de card, 4 botões, TTS, undo — passou a ser um **segundo** lugar para fazer a mesma coisa (produzir revisões SM-2), e sua fila só se enchia quando havia palavras `nextReview <= agora`, o que na prática empurrava o usuário para a prova ou para o glossário. Redundância sem valor próprio.
- **Opções:** **A) Remover a tela** e deixar a revisão acontecer só pelas provas (a "revisão semanal" já cobre o vocabulário recente). **B) Manter as duas** superfícies. **C) Fundir** a revisão dentro da prova como um modo.
- **Fator decisivo:** o pedido explícito do usuário e o princípio de não ter dois caminhos para a mesma mutação. O SM-2 (agendamento, `ReviewLog`, due-ness) é o núcleo e permanece intacto — só a superfície redundante sai.
- **Decisão:** **A.** Removidos a rota `/review`, a UI da sessão (`src/ui/review/*`, `useSpeech`), a máquina de estados `src/domain/srs/reviewSession.ts`, os casos de uso de `src/application/review.ts` e os métodos de repositório que só a serviam (`listDueForReview`, `applyReview`, `revertReview`, `findNextScheduled`). O SM-2 (`sm2.ts`), o `ReviewLog` e os insights de due-ness/streak/forecast ficam — alimentados pelas provas. No Painel, a CTA "Revisar agora" virou "Fazer uma prova" (→ `/exams`); a aba de navegação e o passo do tour saíram.
- **Consequências:** um só caminho para estudar (provas), menos código e superfície de teste. A previsão de 7 dias e a faixa "hoje" seguem informando a due-ness, mas agora convidam a fazer uma prova. Reintroduzir uma revisão avulsa exigiria ressuscitar a fila + a UI — os testes de SM-2 puro seguem cobrindo o núcleo caso isso volte.
...
4. **Fluxo C (prova) ponta a ponta no modo Manual:** gerar prompt → colar JSON → validar → atualizar SRS. ✅ _Evoluído para quiz in-app: primeiro com motor local de questões (ADR-007), depois com a prova inteira gerada pela IA — só múltipla escolha, corrigida localmente (ADR-009); dois turnos permanece só na compreensão._
5. **Fluxo B (revisão):** fila de revisão por data + avaliação. ✅ _Teve uma fase de sessão card-a-card (4 botões, atalhos, virada de card, TTS, undo), depois **aposentada** (ADR-010): a revisão SM-2 agora acontece ao responder provas; o agendamento/`ReviewLog`/due-ness continuam._
6. **Fluxo D (dashboard):** métricas e histórico. ✅ _Painel com faixa "hoje" (streak + revisões devidas), heatmap de atividade, previsão de 7 dias, crescimento do vocabulário, tendência de notas e palavras difíceis (ADR-008)._
7. **Polimento de portfólio:** README com "como rodar" + decisões deste doc, e deploy demo opcional na Vercel.
8. **(Opcional, depois):** `ApiAdapter` ligando a IA via API. ✅ _Implementado: correção automática, definição assistida e enriquecimento de questões._

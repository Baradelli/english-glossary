...
- **Decisão:** **A.** `buildQuizGenerationPrompt` pede uma questão de múltipla escolha por verbete (enunciado, 4 alternativas, índice correto e `explanation` em PT); `parseAiQuiz` valida contra o schema Zod; `buildAiQuizQuestions` (puro, semente injetável) casa cada item ao verbete, descarta itens desonestos (alternativas vazias/duplicadas, termo sem correspondência), **reembaralha as alternativas** (o índice correto da IA nunca vaza pela posição) e numera as questões. A app tenta a geração **duas vezes** antes de falhar com mensagem amigável; timeout de 90 s. Correção, nota e prova de prática permanecem locais e inalterados; a prática agora gera questões **novas** para os erros. O motor local (`generator.ts`, `cloze.ts`) foi removido.
- **Consequências:** provas passam a exigir chave de API (a UI de provas avisa e desabilita os botões sem chave); as questões ficam naturais e ganham explicação didática após responder; o schema ganhou `ExamQuestion.explanation` (migração aditiva). Provas antigas do motor local continuam legíveis e corrigíveis. Reverter significaria ressuscitar o motor local — improvável, dado o motivo da troca.
